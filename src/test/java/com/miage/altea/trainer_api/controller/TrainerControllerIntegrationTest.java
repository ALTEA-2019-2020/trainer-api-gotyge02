...
public class TrainerControllerIntegrationTest {
    ...
    void getTrainers_shouldThrowAnUnauthorized(){
        var responseEntity = this.restTemplate.getForEntity("http://localhost:"+port+"/trainers/Ash", Trainer.class);
    ...
    void createTrainers_shouldCreateNewEntryInDatabase() {
        ...
        Trainer newTrainerWithMewLvl100 = new Trainer("I'm mew",Arrays.asList(new Pokemon(151,100)));
        var trainers= this.restTemplate.withBasicAuth(username,password).postForObject("http://localhost:" + port + "/trainers/", newTrainerWithMewLvl100, Trainer.class);
        assertNotNull(trainers);
    ...
    void updateTrainers_shouldUpdateEntryInDatabase() {
        ...
        this.restTemplate.withBasicAuth(username, password).put("http://localhost:" + port + "/trainers/", trainer, Trainer.class);
        var trainersAfter =this.restTemplate.withBasicAuth(username, password).getForObject("http://localhost:" + port +"/trainers/", Trainer[].class);
        assertNotNull(trainersBefore);
    ...
    void deleteTrainer_shouldDeleteEntryInDatabase() {
        ...
        assertEquals("Ash", trainersBefore[0].getName());
        assertEquals("Misty", trainersBefore[1].getName());
        this.restTemplate.withBasicAuth(username, password).delete("http://localhost:" + port + "/trainers/Ash", Trainer.class);
        var trainersAfter = this.restTemplate.withBasicAuth(username, password).getForObject("http://localhost:" +port + "/trainers/", Trainer[].class);
        assertNotNull(trainersBefore);
